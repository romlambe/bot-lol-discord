...
export const announceMatch = async(channel: TextChannel, match: Match) => {
	...
	const scoreRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
		new ButtonBuilder().setCustomId(`score_${match.pandascore_id}_3-0`).setLabel("3-0").setStyle(ButtonStyle.Secondary),
		new ButtonBuilder().setCustomId(`score_${match.pandascore_id}_3-1`).setLabel("3-1").setStyle(ButtonStyle.Secondary),
		new ButtonBuilder().setCustomId(`score_${match.pandascore_id}_3-2`).setLabel("3-2").setStyle(ButtonStyle.Secondary),
	)
